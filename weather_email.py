...
def build_day_section(d):
    ...
    for i in d["display_idx"]:
        ...
        rows.append(f"<tr style='background:{rain_bg(d[\"pps\"][i])};'>"
                    ...
                    f"<td style='padding:5px 8px;text-align:center;'>{d['winds'][i]:.0f} km/h</td></tr>")
...
WMO_CODES = {
    ...
    55: ("Dense drizzle",            "\U0001f327\ufe0f"), 61: ("Light rain",               "\U0001f327\ufe0f"),
    63: ("Moderate rain",            "\U0001f327\ufe0f"), 65: ("Heavy rain",               "\U0001f327\ufe0f"),
    71: ("Light snow",               "\u2744\ufe0f"),  73: ("Moderate snow",            "\u2744\ufe0f"),
    75: ("Heavy snow",               "\u2744\ufe0f"),  77: ("Snow grains",              "\U0001f328\ufe0f"),
    80: ("Light showers",            "\U0001f326\ufe0f"), 81: ("Moderate showers",         "\U0001f327\ufe0f"),
    82: ("Violent showers",          "\u26c8\ufe0f"), 85: ("Light snow showers",       "\U0001f328\ufe0f"),
    86: ("Heavy snow showers",       "\U0001f328\ufe0f"), 95: ("Thunderstorm",             "\u26c8\ufe0f"),
    96: ("Thunderstorm with hail",   "\u26c8\ufe0f"), 99: ("Thunderstorm, heavy hail", "\u26c8\ufe0f"),
}
...
def wmo_info(code):  return WMO_CODES.get(code, (f"Code {code}", "\U0001f321\ufe0f"))
def wmo_desc(code):  return wmo_info(code)[0]
def wmo_emoji(code): return wmo_info(code)[1]
...
def fetch_calendar_events(today_str):
    ...
    try:
        ...
        for component in cal.walk():
            ...
            if isinstance(sv, datetime.date) and not isinstance(sv, datetime.datetime):
                if sv == today:
                    events.append((summary, None, None))
                continue
...
def detect_conflicts(events, day_of_week):
    ...
    for (title, sv, ev) in events:
        if sv is None or ev is None:
            continue
        ev_start_mins = sv.hour * 60 + sv.minute
        ev_end_mins   = ev.hour * 60 + ev.minute
        for (wsh, wsm, weh, wem, wdesc, person) in windows:
            win_start = wsh * 60 + wsm
            win_end   = weh * 60 + wem
            if ev_start_mins < win_end and ev_end_mins > win_start:
                conflicts.append((title, sv, wdesc, person))
                break
    return conflicts
EOF
...
def build_schedule_section(day_of_week):
    ...
    for (time_str, label, notes_by_day) in ROMY_DAILY:
        note = notes_by_day.get(day_of_week, "")
        text = f"{label}" + (f" <span style='color:#6c757d;font-size:13px;'>({note})</span>" if note else "")
        rows.append(
            f"<tr><td style='padding:6px 10px;width:70px;font-weight:bold;color:#495057;"
            f"white-space:nowrap;'>{time_str}</td>"
            f"<td style='padding:6px 10px;'>{text}</td></tr>"
        )
    laura = LAURA_EXERCISE.get(day_of_week)
    aaron = AARON_NOTES.get(day_of_week)
    aside = ""
    if laura or aaron:
        aside_parts = []
        if laura:
            aside_parts.append(f"<b>Laura:</b> {laura[0]} &mdash; {laura[1]}")
        if aaron:
            aside_parts.append(f"<b>Aaron:</b> {aaron}")
        aside = (
            "<div style='margin:12px 0 0;padding:10px 14px;background:#e9f5ff;"
            "border-left:4px solid #3a9fd1;border-radius:4px;font-size:14px;'>"
            + " &nbsp;|&nbsp; ".join(aside_parts) + "</div>"
        )
    return (
        "<div style='margin:24px 0;'>"
        "<h2 style='font-family:sans-serif;font-size:18px;color:#2c3e50;margin:0 0 10px;'>"
        "&#128197; Today's Schedule</h2>"
        "<table style='border-collapse:collapse;width:100%;background:#f8f9fa;"
        "border-radius:8px;overflow:hidden;font-family:sans-serif;font-size:14px;'>"
        "<tbody>" + "".join(rows) + "</tbody></table>" + aside + "</div>"
...
def build_conflicts_section(events, conflicts):
    ...
    if events:
        ev_rows = []
        for (title, sv, ev) in events:
            time_str = "All day" if sv is None else f"{sv.strftime('%H:%M')}–{ev.strftime('%H:%M')}"
            ev_rows.append(
                f"<tr><td style='padding:5px 10px;width:100px;font-weight:bold;"
                f"white-space:nowrap;color:#495057;'>{time_str}</td>"
                f"<td style='padding:5px 10px;'>{title}</td></tr>"
            )
        sections.append(
            "<b style='font-size:14px;color:#2c3e50;'>&#128197; Google Calendar — Today</b>"
            "<table style='border-collapse:collapse;width:100%;margin-top:6px;"
            "font-family:sans-serif;font-size:14px;background:#f0f7ff;border-radius:6px;'>"
            "<tbody>" + "".join(ev_rows) + "</tbody></table>"
        )
    ...
    if conflicts:
        warn_rows = []
        for (title, sv, wdesc, person) in conflicts:
            time_str = sv.strftime("%H:%M") if sv else "?"
            warn_rows.append(
                f"<tr style='background:#fff3cd;'>"
                f"<td style='padding:6px 10px;width:80px;font-weight:bold;"
                f"white-space:nowrap;color:#856404;'>{time_str}</td>"
                f"<td style='padding:6px 10px;color:#856404;'>"
                f"<b>{title}</b> clashes with <em>{wdesc}</em> ({person})</td></tr>"
            )
        sections.append(
            "<b style='font-size:14px;color:#856404;margin-top:14px;display:block;'>"
            "&#9888;&#65039; Calendar Conflicts</b>"
            "<table style='border-collapse:collapse;width:100%;margin-top:6px;"
            "font-family:sans-serif;font-size:14px;border-radius:6px;overflow:hidden;'>"
            "<tbody>" + "".join(warn_rows) + "</tbody></table>"
    ...
    return (
        "<div style='margin:24px 0;padding:14px;background:#ffffff;"
        "border:1px solid #dee2e6;border-radius:8px;'>"
        + "<div style='margin-bottom:14px;'>".join(sections) + "</div></div>"
    )
...
def build_day_section(d):
    ...
    rows = []
    for i in d["display_idx"]:
        t = d["times"][i]; h = int(t[11:13])
        bg = rain_bg(d["pps"][i])
        rows.append(
            f"<tr style='background:{bg};'>"
            f"<td style='padding:5px 8px;font-weight:bold;white-space:nowrap;'>{fmt_hour(h)}</td>"
            f"<td style='padding:5px 8px;'>{wmo_emoji(d['codes'][i])} {wmo_desc(d['codes'][i])}</td>"
            f"<td style='padding:5px 8px;text-align:center;background:{temp_color(d['temps'][i])};'>"
            f"{d['temps'][i]:.0f}°C</td>"
            f"<td style='padding:5px 8px;text-align:center;color:#6c757d;'>{d['feels'][i]:.0f}°C</td>"
            f"<td style='padding:5px 8px;text-align:center;'>{d['pps'][i]:.0f}%</td>"
    ...
    return (
        ...
        f"<div style='padding:14px 16px;background:#2c3e50;border-radius:8px 8px 0 0;'>"
        f"<span style='font-size:28px;'>{wmo_emoji(d['wmo'])}</span>"
        f"<span style='color:#fff;font-size:16px;margin-left:10px;vertical-align:middle;'>"
        f"<b>{d['conditions']}</b></span></div>"
        f"<div style='padding:12px 16px;background:#f8f9fa;border:1px solid #dee2e6;'>"
        f"<table style='width:100%;border-collapse:collapse;font-family:sans-serif;font-size:14px;'><tbody>"
        f"<tr><td style='padding:4px 8px;width:50%;'>&#127777;&#65039; <b>{d['min_temp']:.0f}°C – {d['max_temp']:.0f}°C</b>"
        f" &nbsp;<span style='color:#6c757d;font-size:13px;'>feels {d['feels_min']:.0f}–{d['feels_max']:.0f}°C</span>"
        f"</td><td style='padding:4px 8px;'>&#127774; Sunrise {d['sunrise']} &nbsp; &#127769; Sunset {d['sunset']}</td></tr>"
        f"<tr><td style='padding:4px 8px;'><span style='background:{uv_bg};padding:2px 8px;border-radius:4px;font-size:13px;'>"
        f"UV {d['uv_max']:.0f} — {uv_txt}</span></td>"
        f"<td style='padding:4px 8px;'>&#128168; Wind max {d['wind_max_daily']:.0f} km/h</td></tr>"
        f"<tr><td style='padding:4px 8px;' colspan='2'>&#128062; Best walk windows: <b>{walk_str}</b></td></tr>"
        f"<tr><td style='padding:4px 8px;color:#c0392b;' colspan='2'>&#9928;&#65039; Heavy rain: {bad_str}</td></tr>"
        f"</tbody></table></div>"
        f"<div style='overflow-x:auto;'><table style='border-collapse:collapse;width:100%;font-family:sans-serif;font-size:13px;'>"
        f"<thead><tr style='background:#495057;color:#fff;'>"
        f"<th style='padding:6px 8px;text-align:left;'>Time</th>"
        f"<th style='padding:6px 8px;text-align:left;'>Conditions</th>"
        f"<th style='padding:6px 8px;text-align:center;'>Temp</th>"
        f"<th style='padding:6px 8px;text-align:center;'>Feels</th>"
        f"<th style='padding:6px 8px;text-align:center;'>Rain%</th>"
        f"<th style='padding:6px 8px;text-align:center;'>Wind</th>"
        f"</tr></thead><tbody>" + "".join(rows) + "</tbody></table></div></div>"
    )
...
def build_weekly_summary(data, dates):
    ...
    for ds in dates:
        ...
        pp  = data["daily"]["precipitation_probability_max"][d]
        label = datetime.date.fromisoformat(ds).strftime("%a %-d")
        bg = rain_bg(pp)
        cells.append(
            f"<td style='text-align:center;padding:10px 6px;background:{bg};border-right:1px solid #dee2e6;'>"
            f"<div style='font-size:11px;color:#6c757d;'>{label}</div>"
